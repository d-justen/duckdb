...
TEST_CASE("Prefix range filter direct compression uses one range for contiguous values", "[optimizer]") {
	...
	REQUIRE(info.mode == PrefixRangeCompressionMode::DIRECT_RANGES);
...
TEST_CASE("Prefix range filter keeps signed ranges crossing zero as bitmaps", "[optimizer]") {
	DuckDB db(nullptr);
	Connection con(db);

	auto filter = BuildInt32PrefixRangeFilter(*con.context, {-2, 5, 6}, -2, 6, 0, 0);
	auto info = filter->GetCompressionInfo();
	REQUIRE(info.mode == PrefixRangeCompressionMode::BITMAP);
	REQUIRE(ContainsKey(*filter, -2));
	REQUIRE(ContainsKey(*filter, 5));
...
TEST_CASE("Prefix range filter compression preserves randomized build keys", "[optimizer]") {
	DuckDB db(nullptr);
	Connection con(db);
	...
	for (idx_t i = 0; i < 2000; i++) {
		...
		state ^= state >> 17;
		state ^= state << 5;
		keys.push_back(UnsafeNumericCast<int32_t>(state % 20001) - 10000);
	}

	auto filter = BuildInt32PrefixRangeFilter(*con.context, keys, -10000, 10000, 7, 0.25);
	for (auto key : keys) {
		REQUIRE(ContainsKey(*filter, key));
	}
}
