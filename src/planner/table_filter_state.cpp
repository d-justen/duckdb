...
public:
	...
	idx_t FilterSelection(SelectionVector &sel, Vector &vector, UnifiedVectorFormat &vdata, idx_t scan_count,
	                      idx_t &approved_tuple_count) override {
		for (auto &child : children) {
			child->FilterSelection(sel, vector, vdata, scan_count, approved_tuple_count);
...
static unique_ptr<ExpressionFilterExecutor> TryCreateComparisonExecutor(const BoundFunctionExpression &func) {
	...
	const auto constant_type = constant->value.type().InternalType();
	if (column_type != constant_type || !IsSupportedComparisonType(column_type)) {
		return nullptr;
	}
	return make_uniq<ComparisonFilterExecutor>(comparison_type, constant->value);
}

static unique_ptr<ExpressionFilterExecutor> TryCreateFunctionExecutor(const BoundFunctionExpression &func,
                                                                      bool inside_selectivity_optional) {
	if (func.children.size() == 2 && BoundComparisonExpression::IsComparison(func.GetExpressionType())) {
	...
	if (func_name == BloomFilterScalarFun::NAME) {
		...
		return make_uniq<BloomFilterExecutor>(func.bind_info->Cast<BloomFilterFunctionData>(),
	...
	if (func_name == PrefixRangeScalarFun::NAME) {
		...
		return make_uniq<PrefixRangeFilterExecutor>(func.bind_info->Cast<PrefixRangeFunctionData>(),
		                                            inside_selectivity_optional);
	}
	return nullptr;
}
...
static unique_ptr<ExpressionFilterExecutor> TryCreateFastExecutor(const Expression &expression,
	...
	case ExpressionClass::BOUND_CONJUNCTION: {
		...
		vector<unique_ptr<ExpressionFilterExecutor>> children;
		for (auto &child_expr : conjunction.children) {
