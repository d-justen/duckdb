...
struct PRFRunResult {
	...
	idx_t shift = 0;
	idx_t range_count = 0;
	double estimated_false_positive_rate = 0;
...
BloomRunResult RunBloomFilterBenchmark(ClientContext &context, const vector<uint64_t> &build_keys,
	...
	result.build_core_ns = TimeNs([&]() {
		...
		filter.InsertHashes(build_hashes);
	});
...
PRFRunResult RunPrefixRangeFilterBenchmark(ClientContext &context, const vector<uint64_t> &build_keys,
	...
	result.build_core_ns = TimeNs([&]() {
		filter->InsertKeys(build_vector, *build_state);
	...
	result.estimated_false_positive_rate = info.false_positive_rate;
	result.active_buckets = analysis.active_buckets;
	result.bytes = info.mode == PrefixRangeCompressionMode::DIRECT_RANGES
...
void WriteHeader(std::ostream &out) {
	out << "filter,cluster_count,build_time_ns,post_processing_time_ns,summary_bytes,probe_throughput_per_sec,"
	       "false_positives,estimated_false_positive_rate,actual_false_positive_rate,mode,range_count,shift,"
	       "active_buckets\n";
}
...
string FormatBloomRow(const BenchmarkConfig &config, idx_t cluster_count, const MembershipInfo &membership,
	...
	const auto actual_false_positive_rate =
	    ComputeActualFalsePositiveRate(result.false_positive_count, membership, config.probe_count);
	...
	out << "bloom," << cluster_count << ',' << result.build_core_ns << ',' << 0 << ',' << result.bytes << ','
	    ...
	    << ',' << std::setprecision(9) << actual_false_positive_rate << ',' << "na,0,0,0";
	return out.str();
}

const char *CompressionModeName(PrefixRangeCompressionMode mode) {
	switch (mode) {
	case PrefixRangeCompressionMode::BITMAP:
		return "bitmap";
	case PrefixRangeCompressionMode::DIRECT_RANGES:
...
string FormatPRFRow(const char *filter_name, const BenchmarkConfig &config, idx_t cluster_count,
	...
	out << filter_name << ',' << cluster_count << ',' << result.build_core_ns << ',' << post_build_ns << ','
	    ...
	    << result.false_positive_count << ',' << std::setprecision(9) << result.estimated_false_positive_rate << ','
	    << actual_false_positive_rate << ',' << CompressionModeName(result.mode) << ',' << result.range_count << ','
	    << result.shift << ',' << result.active_buckets;
	return out.str();
}
